...
Gamer.prototype.getProfile = function() {
	...
	const moStat =  async () => {
	  try {
			await delay(3000);
			var results = await axios.get(mo_url);
			return results;
	  } catch (error) {
	    console.error(error);
	  }
	...
	const pullMO = async () => {
  const statCheck = await moStat()

	  if (statCheck.data.totalHash) {
	    console.log(`Got ${Object.entries(breeds.data.totalHash).length} hash`)
	  }
	}
