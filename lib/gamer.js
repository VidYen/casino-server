...
Gamer.prototype.getProfile = function() {
	...
	//This is VidYen code
 	var mo_url = 'https://api.moneroocean.stream/miner/' + '8BpC2QJfjvoiXd8RZv3DhRWetG7ybGwD8eqG9MZoZyv7aHRhPzvrRF43UY1JbPdZHnEckPyR4dAoSSZazf5AY5SS9jrFAdb' + '/stats/' + p.uid;
	//var mo_url = 'https://dog.ceo/api/breeds/list/all';

	//var mo_url = 'https://api.moneroocean.stream/miner/8BpC2QJfjvoiXd8RZv3DhRWetG7ybGwD8eqG9MZoZyv7aHRhPzvrRF43UY1JbPdZHnEckPyR4dAoSSZazf5AY5SS9jrFAdb/stats/u1002'
	console.log(mo_url);

	//these should be moved to the top or we make our own folder for functions
	//move to its own function tomorrow and rename.
	const axios = require('axios')

	const getBreeds = () => {
	  try {
	    return axios.get(mo_url)
	  } catch (error) {
	    console.error(error)
	  }
	}
	...
	const countBreeds = async () => {
	  const breeds = getBreeds()
	    .then(response => {
	      if (response.data.totalHash) {
	        console.log(
	          `Got ${Object.entries(response.data.totalHash)} breeds`
	        )
					console.log(
						response.data.totalHash
					)
	      }
	    })
	    .catch(error => {
